...
to setup-network ;; The social network is determined by the number of communities, number of members per community, clustering component, rewiring-proportion, dekkers-power.
  ...
  if network-design = "Connected-Caveman" [
    ...
    let index 1
    let members-per-community (total-population / n-communities)
      repeat n-communities [
        nw:generate-small-world people connections ceiling sqrt members-per-community ceiling sqrt members-per-community clustering-exponent true [
          set my-community index
          set size 3
          set color blue
        ]
      ;Afterwards each community is connected by the previous and proceeding community by adding one random link between nodes within both communities.
...
to setup-tipping-scenario ;;; Assigns which agents are the social entrepreneurs (green) at the start of the simulation. Based on Centola (2018) you choose between Shotgun (assinging the
...
tipping-strategy
tipping-strategy
"Snowball" "Shotgun" "Silver-Bullets" "Outskirts"
0
