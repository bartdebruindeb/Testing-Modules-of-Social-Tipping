...
#df3 <- rbind(df1, df2)
write.csv(clean_df,"C:/Users/20225262/OneDrive - TU Eindhoven/Documents/Models/Social Tipping Modules/Probalistic/NWG_clean.csv" )
...
```{r echo=FALSE,fig.height=7, fig.width=7, message=TRUE, warning=TRUE, results='asis'}
#df_mean$Majority_Green <- as.factor(df_mean$Majority_Green)
# "Mean_Average_Path", #Connections","Mean_Node_Degree", "Mean_Cluster_Coefficient"

clean_df$Rewiring_Proportion <- as.factor(clean_df$Rewiring_Proportion)
df_use <- filter(clean_df, clean_df$Network_Generator != "custom", clean_df$MPL != "false") 
df_use_2 <- clean_df %>% filter(Network_Generator == "custom", MPL != "false") %>%
   dplyr::sample_frac()
...
#  geom_point(data = df_use_2, aes(x = MPL, y = M, fill= VD), size = 1)+
 #scale_color_gradient(low = "blue", high = "red", na.value = NA, name = "Var Node Degree")+
# ggnewscale::new_scale_color() +
   geom_point(data = clean_df, aes(x = MPL, y = GCC, size = Network_Generator, shape = Network_Generator, color = Network_Generator, fill= VD)) +
